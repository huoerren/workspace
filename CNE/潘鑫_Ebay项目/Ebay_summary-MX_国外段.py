...
def forMX_02(received_time, complete_time):  # 周一到周五及周六上午半天
    if (pd.isnull(received_time) or pd.isnull(complete_time)):
        ...
        # case02:开始时间和结束时间不是在同一天
            ...
            while (received_time.strftime('%Y-%m-%d') <= (complete_time + datetime.timedelta(days=-1)).strftime(
                    '%Y-%m-%d')):
                if (received_time).weekday() == 5:
                    count += 1
                received_time = received_time + datetime.timedelta(days=1)
...
def getAcount(i):
    ...
    #   落地 - 清关（海关放行）
    d_luodi_qingguan = pd.merge(df_luodi, df_haiguangfangxing[["内单号", "放行时间"]], left_on="内单号", right_on="内单号",
                                how='inner')
    ...
    #   清关- 抵达末端派送中心（交付）
    d_didaPaisongCen_haiguangfangxing = pd.merge(df_haiguangfangxing, df_didaPaisongCen[["内单号", "抵达派送中心时间"]],
                                                 left_on="内单号", right_on="内单号", how='inner')
    ...
    d_didaPaisongCen_haiguangfangxing['用时'] = d_didaPaisongCen_haiguangfangxing.apply(
        lambda x: forMX(x['放行时间'], x['抵达派送中心时间']), axis=1)  # 针对MX
    ...
    #    交付 - 妥投
    d_jiaofu_tuotou = pd.merge(df_didaPaisongCen, df_tuotou[["内单号", "妥投时间"]], left_on="内单号", right_on="内单号",
                               how='inner')
