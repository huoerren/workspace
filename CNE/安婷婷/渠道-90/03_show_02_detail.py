...
zhouqi_tru = list(set(zip(d10t['周序数'] , d10t['周期'])))
        ...
        Line({"theme": ThemeType.MACARONS})
            ...
            .add_yaxis("KPI",lk , label_opts=opts.LabelOpts(is_show=False),
                       markpoint_opts=opts.MarkPointOpts(
                           data=[opts.MarkPointItem(name="KPI", coord=[lx[0], lk[0]], value=lk[0])]),linestyle_opts=opts.LineStyleOpts(color="#6959CD", width=2 )
                       )
			.add_yaxis("期望D",lq , label_opts=opts.LabelOpts(is_show=False),
                       markpoint_opts=opts.MarkPointOpts( data=[opts.MarkPointItem(name="期望D", coord=[lx[0], lq[0]], value=lq[0])]),
                        linestyle_opts=opts.LineStyleOpts(color="#C9B66D", width=2)
                       )
            .extend_axis(yaxis=opts.AxisOpts())
            .add_yaxis('妥投率', listE, is_smooth=True, yaxis_index=1,linestyle_opts=opts.LineStyleOpts(color="#A1343C", width=2 ) )
            .set_colors(['#6FB0AC','#6959CD','#C9B66D','#A1343C'])
            .set_global_opts(
...
#     for z in dca['主题']:
#         zt = z
#         df = d11[(d11['主题'] == z) & (d11['渠道'].isin(qudao_opt) ) & ( d11['周期'].isin(zhouqi_opt) )]
#         if df.size == 0:
#             pass
#         else:
#             line = p_Line(df, zt)
...
for qd in set(d10t['渠道']):
    ...
    for z in dca1['主题']:
        zt=z
        df=d11t[ (d11t['主题']==z) & (d11t['渠道'].isin(qudao_opt) ) & ( d11t['周期'].isin(zhouqi_opt)) &( d11t['国家'].isin(guojia_opt) ) ]
        if df.size == 0:
            pass
        else:
