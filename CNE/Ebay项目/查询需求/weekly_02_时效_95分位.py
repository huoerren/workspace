...
def getAllShiXiao():
    ...
    df_zhuangche_qifei = pd.read_csv('03_出库-起飞.csv')
    ...
    for df1, df2 in df_ruku_chuku.groupby(['channel_name', 'des']):
        ...
        dataframe01.append(['首扫-封袋', df1, df3.shape[0], df3.describe(percentiles=[0.95])['用时']['95%']] )
    df_shousao_fengdai = pd.DataFrame(dataframe01, columns=['阶段','分类','数量','95分位用时'] )
    ...
    #     dataframe02.append(['封袋-装车' , df1, df3.shape[0], df3.describe(percentiles=[0.9])['用时']['90%']] )
    ...
    df_zhuangche_qifei['封袋时间'] = pd.to_datetime(df_zhuangche_qifei['封袋时间'])
    df_zhuangche_qifei['起飞时间'] = pd.to_datetime(df_zhuangche_qifei['起飞时间'])

    df_zhuangche_qifei["用时"] = (df_zhuangche_qifei["起飞时间"] - df_zhuangche_qifei["封袋时间"]).astype('timedelta64[s]')
    df_zhuangche_qifei["用时"] = round(df_zhuangche_qifei["用时"] / 86400, 2)

    dataframe03 = []
    for df1, df2 in df_zhuangche_qifei.groupby(['channel_name', 'des']):
        df3 = df2.sort_values(by=["用时"], ascending=True)
        dataframe03.append([ '出库-起飞', df1,  df3.shape[0], df3.describe(percentiles=[0.95])['用时']['95%']] )
    df_zhuangche_qifei = pd.DataFrame(dataframe03, columns=['阶段','分类', '数量', '95分位用时'])
    ...
    df_qifei_luodi = pd.DataFrame(dataframe04, columns=['阶段','分类', '数量', '95分位用时'])
    ...
    df_luodi_qingguan = pd.DataFrame(dataframe05, columns=['阶段','分类', '数量', '95分位用时'])
    ...
    df_qingguan_jiaofu = pd.DataFrame(dataframe06, columns=['阶段','分类', '数量', '95分位用时'])
    ...
    df_jiaofu_tuotou = pd.DataFrame(dataframe07, columns=['阶段','分类', '数量', '95分位用时'])
    ...
    for df1, df2 in df_shousao_tuotou.groupby(['channel_name', 'des']):
        df3 = df2.sort_values(by=["用时"], ascending=True)
        # print(df3.describe(percentiles=[0.9])['用时'] )
        dataframe09.append(['首扫-妥投', df1, df3.shape[0], df3.describe(percentiles=[0.95])['用时']['95%']])

    df_shousao_tuotou_2 = pd.DataFrame(dataframe09, columns=['阶段', '分类', '数量', '95分位用时'])
    print(df_jiaofu_tuotou)

    # print('**************** 首扫-妥投 : end -----------')

    print('--------------  合并最后的结果 ：  -----------')
    frames = [df_shousao_fengdai , df_zhuangche_qifei,df_qifei_luodi,df_luodi_qingguan,df_qingguan_jiaofu,df_jiaofu_tuotou,df_shousao_tuotou_2]
