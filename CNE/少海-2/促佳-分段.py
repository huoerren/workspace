...
days = "BETWEEN '2021-12-05 16:00:00' and '2021-12-12 16:00:00' "
print(days)
...
danliang3 = danliang.groupby(['channel_code', 'des','周序数','周期'])['单量'].sum().reset_index()
danliang1 = danliang.groupby(['channel_code', 'des','周序数','周期', 'moon'])['单量'].sum().reset_index()
danliang2 = danliang1.groupby(['channel_code', 'des', 'moon'])['单量'].sum().reset_index()
# dck  入库-出库
# dzc   出库--装车
# dqf   装车--起飞
# dl    起飞--落地
...
for index,row in danliang3.iterrows():
    for t in list6:
