    ...
    if file_path is not None:
        ...
        try:
            ...
            if (is_checked0.get()):
                ...
                d0 = pd.DataFrame()
                d0['内单号'] = df['内单号']
                d0 = pd.merge(d0, d01, on=['内单号'], how='outer')
                d0 = pd.merge(d0, d02, on=['内单号'], how='outer')
                d0 = pd.merge(d0, d03, on=['内单号'], how='outer')
                d0['出口清关时间'] = d0.apply(lambda x: judge(x['主单出口清关时间'], x['包裹出口清关时间'], x['正式单出口清关时间']), axis=1)
                df = pd.merge(df, d0[['内单号', '出口清关时间']], on=['内单号'], how='left')
            if (is_checked5.get()):
                #起飞
                S5 = """
                    SELECT 
...
ck4 = tk.Checkbutton(frm, text='首扫(入库)',variable=is_checked4)
ck1 = tk.Checkbutton(frm, text='装车(出库)',variable=is_checked)
ck2 = tk.Checkbutton(frm, text='封袋',variable=is_checked1)
ck0 = tk.Checkbutton(frm, text='清关(出口)',variable=is_checked0)
ck5 = tk.Checkbutton(frm, text='起飞',variable=is_checked5)
ck6 = tk.Checkbutton(frm, text='落地',variable=is_checked6)
ck7 = tk.Checkbutton(frm, text='清关(进口)',variable=is_checked7)
...
ck0.grid(row=0, column=3) #0出口清关
ck1.grid(row=0, column=2) #3装车
ck2.grid(row=0, column=1) #2封袋
ck3.grid(row=0, column=7) #7交付
ck4.grid(row=0) #1首扫
ck5.grid(row=0, column=4) #4起飞
ck6.grid(row=0, column=5) #5落地
ck7.grid(row=0, column=6) #6清关
ck8.grid(row=0, column=8) #8妥投
ck9.grid(row=0, column=9) #9加装车-全
frm.grid(row=2)
