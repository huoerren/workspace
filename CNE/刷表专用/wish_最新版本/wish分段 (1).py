...
# 起飞监控
s4 = """select  count(distinct order_no) c, 
lgm.mawb_no,
lgo.channel_code, 
date_format(date_add(lgo.gmt_create,interval 8 hour),'%Y-%m-%d') gmtdate,
date_format(date_add(tbe.event_time,interval 8 hour),'%Y-%m-%d') zcdate,
...
where  
tbe.event_code="DEPS" 
AND tme.event_code in("SDFO","DEPC","DEPT","LKJC",'SYFD','SYYF')
and lgo.gmt_create {}
...
track_mawb_event tme1
where
tme1.event_code in("SDFO","DEPC","DEPT","LKJC",'SYFD','SYYF')
and tme1.mawb_id=lgo.mawb_id
)
group by 
lgm.mawb_no, zcdate, qfdate, gmtdate, fddate, lgo.channel_code, lgo.des,装车用时,起飞用时,业务至起飞用时
...
and tme2.event_code in("SDFO","DEPC","DEPT","LKJC",'SYFD','SYYF') 
and lgo.gmt_create {}
and lgo.platform='WISH_ONLINE' 
AND lgo.customer_id in  (1151368,1151370,1181372,1181374) 
and lgo.order_status in(1,2) 
...
select 1
from 
track_mawb_event tme3
where
tme3.event_code='PMWC'
and tme3.mawb_id=lgo.mawb_id
)
group by 
2,3,4,5,6,7,8,9,10
union all
...
track_mawb_event tme3
where
tme3.event_code in("SDFO","DEPC","DEPT","LKJC",'SYFD','SYYF') 
and tme3.mawb_id=lgo.mawb_id
)
group by 
2,3,4,5,6,7,8,9,10
...
lg_bag lgb,
lg_bag_order_relation lbor 
where tme.event_code in ("IRCM","PVCS","IRCN","RFIC",'BGRK','RHCC','RHIR','RFES','RELE') 
and lgo.gmt_create {}
and platform='WISH_ONLINE' 
AND customer_id in  (1151368,1151370,1181372,1181374) 
...
and lgo.id=lbor.order_id 
and lbor.bag_id=lgb.id 
AND toe.event_code in ("IRCM","PVCS","IRCN","RFIC",'BGRK','RHCC','RHIR','RFES','RELE') 
AND lgo.order_status =2 
and lgo.gmt_create {} 
and lgo.is_deleted='n' 
and lgm.is_deleted='n' 
...
and lbor.order_id=lgo.id 
and lgo.mawb_id=lgm.id 
AND tbe.event_code in ("IRCM","PVCS","IRCN","RFIC",'BGRK','RHCC','RHIR','RFES','RELE') 
and lgo.gmt_create {} 
and lgo.platform='WISH_ONLINE' 
AND lgo.customer_id in (1151368,1151370,1181372,1181374) 
and lgo.order_status =2 
...
where  tbe.event_code in ("JFMD","AAPS") #派送公司收货 
and lgo.gmt_create {}
and olt.event_time {} 
and lgo.platform='WISH_ONLINE' 
AND lgo.customer_id in (1151368,1151370,1181372,1181374)   
...
['交付节点', djf], ['断更监控', ddg], ['交付监控', dj], ['落地节点', dll]]


def file_xlsx(name, df):
    bf = r'F:\PBI临时文件\wish分段监控\{}.xlsx'.format(name)
