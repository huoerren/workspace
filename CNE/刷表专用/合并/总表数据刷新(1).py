...
order by c desc)
UNION ALL
(SELECT channel_code "渠道名称",des "目的地", date_format(date_add(gmt_create,interval 8 hour),'%Y-%m-%d') as "业务日期" ,
 customer_id,round(timestampdiff(hour,lgo.gmt_create,lgo.delivery_date)/24,1) as delivery_interval,order_status "追踪状态" ,ISNULL( mawb_id ), count(1) c 
from lg_order lgo 
where gmt_create {}
and platform='JDW'
...
s4="""(SELECT order_no,date_add(gmt_create,interval 8 hour) datetime ,customer_id 
from lg_order lgo
where 
# gmt_create>  '2021-2-28 16:00:00'
gmt_create>  '2021-4-30 16:00:00'
