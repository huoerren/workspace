...
date_format(date_add(tme.event_time,interval 8 hour),'%Y-%m-%d') qfdate,
date_format(date_add(lgo.gmt_create,interval 8 hour),'%Y-%m-%d') ywdate,
round(timestampdiff(hour,tme.event_time,tbe.event_time)/24,1) 起飞到落地,
round(timestampdiff(hour,lgo.gmt_create,tbe.event_time)/24,1) 业务到落地,
( CASE WHEN lgo.customer_id = 4691294 THEN "TT UK" ELSE "Perceiver Limited" END ) cusid,
( CASE WHEN lgo.order_status = 2 THEN "转运中" ELSE "其他" END ) STATUS 
from
track_bag_event tbe,
...
date_format(date_add(tme.event_time,interval 8 hour),'%Y-%m-%d') qfdate,
date_format(date_add(lgo.gmt_create,interval 8 hour),'%Y-%m-%d') ywdate,
round(timestampdiff(hour,tme.event_time,toe.event_time)/24,1) 起飞到落地,
round(timestampdiff(hour,lgo.gmt_create,toe.event_time)/24,1) 业务到落地,
( CASE WHEN lgo.customer_id = 4691294 THEN "TT UK" ELSE "Perceiver Limited" END ) cusid,
( CASE WHEN lgo.order_status = 2 THEN "转运中" ELSE "其他" END ) STATUS
from lg_order lgo,
...
# dfland = pd.read_excel(u"C:\\Users\\Administrator\\Desktop\\TT DEMO\\5落地监控.xlsx")

# dfland['c']=dfland['c'].astype('int')
# dfl = dfland.groupby(['mawb_no', 'bag_no', 'channel_code', 'des', 'lddate', 'qfdate', 'ywdate', '起飞到落地','业务到落地'，'cusid'，'STATUS'])['c'].sum().reset_index()
# print(dfl)

# dfl.to_excel(excel_writer=r"C:\Users\Administrator\Desktop\TT DEMO\落地节点.xlsx",index = False)
...
# dfd = pd.read_excel(u"C:\\Users\\Administrator\\Desktop\\TT DEMO\\交付节点.xlsx")
...
# dfdist.to_excel(excel_writer=r"C:\Users\Administrator\Desktop\TT DEMO\7交付监控.xlsx",index = False)
...
dfcust1 = dfcust.groupby(['mawb_no','fxdate']).sum().reset_index()
print('清关主单',dfcust1.head())
...
dfland = dfl.groupby(['mawb_no','bag_no', 'channel_code', 'des', 'lddate','ywdate','cusid','STATUS'])['c'].sum().reset_index()
print("落地节点", dfland.head())
...
dfd = exe(s10)
dfd['c'] = dfd['c'].astype('int')
# dfdist = dfd.groupby(['mawb_no', 'bag_no', 'channel_code', 'des', 'jfdate', 'ywdate','cusid','STATUS'])['c'].sum().reset_index()
dfdist = dfd.groupby(['mawb_no','jfdate']).sum().reset_index()
print("交付节点", dfdist.head())
print("交付监控", dfd.head())
...
name_l = [['未配载监控', dfunload], ['出库监控', dfload], ['装车监控', dfdeps], ['起飞监控', dftkoff], ['落地监控', dfl], ['清关全', dfcust], ['主单全', exe(s12)],
['交付节点', dfdist], ['断更监控', dfdlvr], ['交付监控', dfd], ['落地节点', dfland],['清关主单',dfcust1]]
