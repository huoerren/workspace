...
def getAllShiXiao():
    ...
    df_zhuangche_qifei = pd.read_csv('03_装车-起飞.csv')
    ...
    for df1, df2 in df_shousao_tuotou.groupby(['channel_name', 'des']):
        ...
        dataframe10.append(['首扫-妥投', df1, df3.shape[0], df3.describe(percentiles=[0.95])['用时']['95%']])
    df_shousao_tuotou_02 = pd.DataFrame(dataframe10, columns=['阶段', '分类', '数量', '95分位用时'])
    print(df_shousao_tuotou_02)
    ...
    df_fengdai_zhuangche = pd.DataFrame(dataframe02, columns=['阶段','分类','数量','95分位用时'] )
    ...
    print('--------------  合并最后的结果 ：  -----------')
    frames = [ df_shousao_tuotou_02 ,df_shousao_fengdai ,df_fengdai_zhuangche,df_zhuangche_qifei,df_qifei_luodi,df_luodi_qingguan,df_qingguan_jiaofu,df_jiaofu_tuotou ]
