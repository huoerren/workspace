...
def getMatrixByDf(df):
    ...
    heatmap_count = df.pivot("channel_code", "des", "rate")
...
# 获得上周期间 渠道-目的国-平台 单量
def getLastWeekData():
    df = pd.read_csv('lastWeek_channel_des_platform.csv' ,encoding='gbk')
...
def  getBoxplotForWeekCountAndLine(dataframe):
    ...
    # 显示 线型图
    sns.lineplot(x= "weekly" , y="weekCount" , markers= True ,  data=df_sub_02)
