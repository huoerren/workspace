...
def getAllShiXiao():
    ...
    df_shousao_zhuangche  = pd.read_csv('特殊节点_首扫-装车.csv')

    #特殊节点：首扫-装车

    print('**************** 首扫-装车 start  ; *********** ')
    # shoudao_fengdai = pd.merge(shousao, fengdai[["内单号", "封袋时间"]], left_on="内单号", right_on="内单号", how='inner')
    df_shousao_zhuangche['首扫时间'] = pd.to_datetime(df_shousao_zhuangche['首扫时间'])
    df_shousao_zhuangche['装车时间'] = pd.to_datetime(df_shousao_zhuangche['装车时间'])

    df_shousao_zhuangche["用时"] = (df_shousao_zhuangche["装车时间"] - df_shousao_zhuangche["首扫时间"]).astype('timedelta64[s]')
    df_shousao_zhuangche["用时"] = round(df_shousao_zhuangche["用时"] / 86400, 2)

    dataframe08 = []
    for df1, df2 in df_shousao_zhuangche.groupby(['channel_name', 'des']):
        df3 = df2.sort_values(by=["用时"], ascending=True)
        dataframe08.append(['首扫-装车', df1, df3.shape[0], df3.describe(percentiles=[0.9])['用时']['90%']])
    df_shousao_zhuangche_result = pd.DataFrame(dataframe08, columns=['阶段', '分类', '数量', '90分位用时'])
    ...
    for df1, df2 in df_ruku_chuku.groupby(['channel_name', 'des']):
        df3 = df2.sort_values(by=["用时"], ascending=True)
        dataframe01.append(['首扫-封袋', df1, df3.shape[0], df3.describe(percentiles=[0.9])['用时']['90%']] )
    df_shousao_fengdai = pd.DataFrame(dataframe01, columns=['阶段','分类','数量','90分位用时'] )
    ...
    for df1, df2 in df_chuku_zhuangche.groupby(['channel_name', 'des']):
        ...
        dataframe02.append(['封袋-装车' , df1, df3.shape[0], df3.describe(percentiles=[0.95])['用时']['95%']] )
    df_fengdai_zhuangche = pd.DataFrame(dataframe02, columns=['阶段','分类','数量','90分位用时'] )
    ...
    for df1, df2 in df_zhuangche_qifei.groupby(['channel_name', 'des']):
        df3 = df2.sort_values(by=["用时"], ascending=True)
        dataframe03.append([ '装车-起飞', df1,  df3.shape[0], df3.describe(percentiles=[0.95])['用时']['95%']] )
    df_zhuangche_qifei = pd.DataFrame(dataframe03, columns=['阶段','分类', '数量', '90分位用时'])
    print(df_zhuangche_qifei)
    ...
    for df1, df2 in df_qifei_luodi.groupby(['channel_name', 'des']):
        df3 = df2.sort_values(by=["用时"], ascending=True)
        # print(df3.describe(percentiles=[0.9])['用时'] )
        dataframe04.append(['起飞-落地', df1,  df3.shape[0],  df3.describe(percentiles=[0.95])['用时']['95%']])
    df_qifei_luodi = pd.DataFrame(dataframe04, columns=['阶段','分类', '数量', '90分位用时'])
    print(df_qifei_luodi)
    # print('**************** 起飞-落地 : end -----------')
    ...
    for df1, df2 in df_luodi_qingguan.groupby(['channel_name', 'des']):
        df3 = df2.sort_values(by=["用时"], ascending=True)
        dataframe05.append([ '落地-清关' ,df1, df3.shape[0],  df3.describe(percentiles=[0.95])['用时']['95%']] )
    df_luodi_qingguan = pd.DataFrame(dataframe05, columns=['阶段','分类', '数量', '90分位用时'])
    print(df_luodi_qingguan)
    ...
    for df1, df2 in df_qingguan_jiaofu.groupby(['channel_name', 'des']):
        df3 = df2.sort_values(by=["用时"], ascending=True)
        # print(df3.describe(percentiles=[0.9])['用时'] )
        dataframe06.append([ '清关-交付',df1,  df3.shape[0],  df3.describe(percentiles=[0.95])['用时']['95%']] )
    df_qingguan_jiaofu = pd.DataFrame(dataframe06, columns=['阶段','分类', '数量', '90分位用时'])
    print(df_qingguan_jiaofu)
    ...
    for df1, df2 in df_jiaofu_tuotou.groupby(['channel_name', 'des']):
        df3 = df2.sort_values(by=["用时"], ascending=True)
        # print(df3.describe(percentiles=[0.9])['用时'] )
        dataframe07.append(['交付-妥投', df1, df3.shape[0], df3.describe(percentiles=[0.95])['用时']['95%']] )

    df_jiaofu_tuotou = pd.DataFrame(dataframe07, columns=['阶段','分类', '数量', '90分位用时'])
    print(df_jiaofu_tuotou)

    # print('**************** 交付-妥投 : end -----------')

    print('--------------  合并最后的结果 ：  -----------')
    frames = [df_shousao_fengdai ,df_fengdai_zhuangche,df_zhuangche_qifei,df_qifei_luodi,df_luodi_qingguan,df_qingguan_jiaofu,df_jiaofu_tuotou,df_shousao_zhuangche_result]
