...
def getAllShiXiao():
    ...
    for df1, df2 in df_zhuangche_qifei.groupby(['channel_name', 'des']):
        df3 = df2.sort_values(by=["用时"], ascending=True)
        dataframe03.append([ '装车-起飞', df1,  df3.shape[0], df3.describe(percentiles=[0.9])['用时']['90%']] )
    ...
    for df1, df2 in df_qifei_luodi.groupby(['channel_name', 'des']):
        df3 = df2.sort_values(by=["用时"], ascending=True)
        # print(df3.describe(percentiles=[0.9])['用时'] )
        dataframe04.append(['起飞-落地', df1,  df3.shape[0],  df3.describe(percentiles=[0.9])['用时']['90%']])
    ...
    for df1, df2 in df_luodi_qingguan.groupby(['channel_name', 'des']):
        df3 = df2.sort_values(by=["用时"], ascending=True)
        dataframe05.append([ '落地-清关' ,df1, df3.shape[0],  df3.describe(percentiles=[0.9])['用时']['90%']] )
    ...
    for df1, df2 in df_qingguan_jiaofu.groupby(['channel_name', 'des']):
        df3 = df2.sort_values(by=["用时"], ascending=True)
        # print(df3.describe(percentiles=[0.9])['用时'] )
        dataframe06.append([ '清关-交付',df1,  df3.shape[0],  df3.describe(percentiles=[0.9])['用时']['90%']] )
    ...
    for df1, df2 in df_jiaofu_tuotou.groupby(['channel_name', 'des']):
        df3 = df2.sort_values(by=["用时"], ascending=True)
        # print(df3.describe(percentiles=[0.9])['用时'] )
        dataframe07.append(['交付-妥投', df1, df3.shape[0], df3.describe(percentiles=[0.9])['用时']['90%']] )
    ...
    for df1, df2 in df_shousao_tuotou.groupby(['channel_name', 'des']):
        df3 = df2.sort_values(by=["用时"], ascending=True)
        # print(df3.describe(percentiles=[0.9])['用时'] )
        dataframe09.append(['首扫-妥投', df1, df3.shape[0], df3.describe(percentiles=[0.9])['用时']['90%']])

    df_shousao_tuotou_2 = pd.DataFrame(dataframe09, columns=['阶段', '分类', '数量', '90分位用时'])
    print(df_jiaofu_tuotou)

    # print('**************** 首扫-妥投 : end -----------')

    print('--------------  合并最后的结果 ：  -----------')
    frames = [df_shousao_fengdai ,df_fengdai_zhuangche,df_zhuangche_qifei,df_qifei_luodi,df_luodi_qingguan,df_qingguan_jiaofu,df_jiaofu_tuotou,df_shousao_tuotou_2]
