...
data_train = pd.read_csv('688018_train.csv')

print(data_train['2_days_flag'].value_counts())
...
# # sns.jointplot(x= data_train.Survived , y= data_train.Age , data= data_train)
...
# Survived_0 = data_train.Pclass[data_train.ifRise == 0].value_counts() # 未获救
# Survived_1 = data_train.Pclass[data_train.ifRise == 1].value_counts() # 获救
...
# df = pd.DataFrame({u'有':Survived_cabin, u'无':Survived_no_cabin}).transpose()
# df.plot(kind = 'bar', stacked = True)
...
data_test = pd.read_csv(r'688018_test.csv') # 导入测验集数据

y = data_train['2_days_flag'] # 将训练集Survived 数据存储在y中
del data_train['2_days_flag'] # 删除训练集Survived数据

sum_id = data_test['trade_date'] # 存储测试集乘客ID
print('-------------- sum_id ------------')
print(len(sum_id))
...
#删掉无关因素
df = df.drop(['ts_code','trade_date','2_days_flag'],axis=1) # 删除姓名、ID、船票信息、客舱信息，axis=0 删除行，=1 删除列
df = df.dropna()
#缺失数据填充
...
df['vol'] = (df['vol']-df['vol'].min()) /(df['vol'].max()-df['vol'].min())
...
X_train, X_val, y_train,y_val = train_test_split(data_train,y,test_size=0.3, random_state=42) # 划分训练集和验证集
# 以7：3（0.3）将训练集与获救结果随机拆分，随机种子为42
...
pred = pd.DataFrame({'PassengerId': sum_id , 'ifRise': pred}) # 格式化预测结果
pred.to_csv('pred_688108.csv',index=None) # 导出数据
