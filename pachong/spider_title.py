...
df = DataFrame()
columns=['标题','价格','付款人数','总评论数','id','类型']
pa=['0,1','44', '88','132','176','220','264','308']
for i in pa:
    ...
    for j in js['mods']['itemlist']['data']['auctions']:
        name  =j['raw_title']     #标题
        price =j['view_price']    #价格
        idd   =j['nid']    #id
        comment_count=j['comment_count']
        view_sales=""
        try:
            view_sales=j['view_sales']
        except:
            pass
        u=j['detail_url']
        if 'tmall' in u:
            ...
            ty='tb'
        data=[name,price,view_sales,comment_count,idd,ty]
        ser=Series(data,index=columns)
        df=df.append(ser,ignore_index=True)

print(df)

# df.to_excel('辅助数据.xlsx',index=False)
