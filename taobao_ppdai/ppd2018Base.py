...
pd.set_option('display.max_rows',1000000000000) #b就是你要设置显示的最大的行数参数
...
delColumnsPart01 = ['id','emp_title','sub_grade','url','purpose','title','earliest_cr_line','bc_open_to_buy',
                    ...
                    'issue_d' , 'zip_code', 'addr_state','last_pymnt_d','next_pymnt_d','last_credit_pull_d' ]

df = df.drop(delColumnsPart01 ,axis=1)
print(df.shape) # (495242, 72)
...
# 将可以数值化的列进行数值化 (连续值分箱求woe | iv 值必须将对应的 列转换成 数值型)
...
for col in NumberColumns:
    ivValue = bin_distince(df[col], df['loan_status'], n=10)
    if ivValue < 0.05 :
        delColumnsPart04.append(col)
    else:
        dictIV[col] = ivValue

if len(delColumnsPart04) >0:
...
delColumnsPart02 = ['funded_amnt','funded_amnt_inv','installment','total_pymnt','total_bal_ex_mort',
                    ...
                    'tot_hi_cred_lim','tot_cur_bal', 'revol_bal' ,'total_pymnt_inv',
                    'total_rec_prncp','last_fico_range_high','open_il_12m','num_actv_bc_tl','num_actv_rev_tl',
                    'num_bc_sats','num_bc_tl','num_op_rev_tl','num_rev_accts','num_rev_tl_bal_gt_0' ]

df = df.drop(delColumnsPart02,axis=1)
# print(df.corr()) # 计算变量之间的相关系数
# print(df.shape) #(118338, 31)
